...
if playerGui:FindFirstChild("FEEmotesGui") then
	return
end
local shared = ReplicatedStorage:WaitForChild("FEEmotes", 15)
if not shared then
	warn("[FE Emotes] Falta ReplicatedStorage/FEEmotes. Consulta README.md.")
	return
end
...
local C = {
	...
	muted = Color3.fromRGB(158, 158, 167),
...
rounded(navigation, 14)
local navIndicator = make("Frame", {
	Position = UDim2.fromOffset(4, 4),
	Size = UDim2.new(1 / 3, -8, 1, -8),
	BackgroundColor3 = C.selected,
	BorderSizePixel = 0,
}, navigation)
rounded(navIndicator, 10)
local viewButtons = {}
for i, name in ipairs({ "Catálogo", "Favoritos", "Equipados" }) do
...
local stopButton =
	button(now, "■", { Position = UDim2.new(1, -54, 0, 14), TextColor3 = C.error, TextSize = 21 })

local controlsToggle =
	button(now, "≡", { Name = "PlayerControls", Position = UDim2.new(1, -104, 0, 14), TextSize = 24 })
...
switches.Position = UDim2.fromOffset(0, 76)
...
local browseTitle = section(66, false)
local collectionEyebrow = text(browseTitle, "LA BIBLIOTECA", 10, C.muted, { Size = UDim2.new(1, -54, 0, 16) })
local collectionTitle = text(browseTitle, "Encuentra tu ritmo.", 19, C.text, {
...
local collectionCount = text(browseTitle, "06", 22, C.accent, {
...
local status = text(panel, "Listo · R15 recomendado", 11, C.muted, {
...
local function request(action, id)
	remote:FireServer({ action = action, id = id })
end
...
local function toggleFavorite(item, favoriteButton)
	...
	if selectedView == "Favoritos" then
		...
		tween(favoriteButton, { TextColor3 = favorites:contains(item.Id) and C.text or C.muted })
	end
end
...
connect(controlsToggle.Activated, function()
	...
	tween(controlsGroup, { Size = UDim2.new(1, 0, 0, controlsOpen and 128 or 0) }, 0.24)
...
local function selectView(name)
	...
	for i, key in ipairs({ "Catálogo", "Favoritos", "Equipados" }) do
		if key == name then
			tween(navIndicator, { Position = UDim2.new((i - 1) / 3, 4, 0, 4) }, 0.25)
		end
...
connect(remote.OnClientEvent, function(snapshot)
	if destroyed then
		return
	end
